...
All functional changes to the repo require a changelog entry. The updates to the changelog will be generated by the `release-please` at release time. This is why it is necessary to adopt [conventional commits](https://www.conventionalcommits.org) in your commit messages, so that important changes can be detected and used to bump the version number correctly at release time.
...
The best way to get your bug fixed is to provide a focused test case that reproduces the specific issue. Please provide a public repository with a runnable example.
...
The typical way to release package versions is using the PR generated by [release-please](https://www.npmjs.com/package/release-please). The way this tool run is:

- a GitHub action listens for changes to the `main` branch
- when new commits are pushed to `main`, it will generate a release PR. The proposed version of the release is based on the `conventional commits` it finds
- Merging the PR will bump the version number, update the changelog, and create a version tag

Another Github action is listening for new version tags. With the new tag in place, this will trigger the publish of the package to NPM.

The manual process to publish packages is wrapped up with a few top level commands:
