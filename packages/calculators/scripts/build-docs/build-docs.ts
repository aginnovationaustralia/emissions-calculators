...
class BuildConstantsDocs {
    ...
    async buildConstantsDocs() {
        const inputPath = path.resolve(appRoot, path.join(this.pathToVersion, 'constants', 'constant_values.ts'));
        ...
        const sections = nodesConstantsProperties.map(property => this.literalToDocSection(property, jsDocCommentsFromESNode)).filter((section) => section !== null)
...
${lines}
    `
        ...
        const outputPath = path.resolve(appRoot, path.join(this.pathToVersion, 'doc', 'CONSTANTS.md'));
    ...
    propertyToConstantValues(property: TSESTree.Property, parents: string[], commentsFromNode: (node: TSESTree.Node) => DocComment[]): ConstantValue[] {
        const value = property.value;
    
        if (value.type === 'ObjectExpression') {
            return value.properties.filter(p => p.type === 'Property').flatMap(p => this.propertyToConstantValues(p, [...parents, this.nameOfKey(property.key)], commentsFromNode));
    ...
    literalToDocSection(literal: TSESTree.ObjectLiteralElement,  commentsFromNode: (node: TSESTree.Node) => DocComment[]): DocSection | null {
