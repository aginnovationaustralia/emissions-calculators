...
export const calculateScope3Fertiliser = (crop: GrainsCropTransformed, context: ExecutionContext<ConstantsForGrainsCalculator>) => {
    const zeroCO2 = rootOrigin(mass('CO2e', new Decimal(0)), { name: 'zero', valueType: 'constant' });
    const fertiliser: Output<3, 'CO2e'> = output('fertiliser', 3, zeroCO2);
    return {
        fertiliser,
    }
}