...
export const calculateScope3EmissionsFromFuel = (crop: GrainsCropTransformed, context: ExecutionContext<ConstantsForGrainsCalculator>) => {
    const zeroCO2 = rootOrigin(mass('CO2e', new Decimal(0)), { name: 'zero', valueType: 'constant' });
    const fuel: Output<3, 'CO2e'> = output('fuel', 3, zeroCO2);
    return {
        fuel,
    }
}