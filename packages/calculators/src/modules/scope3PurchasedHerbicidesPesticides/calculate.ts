...
export const calculateScope3Herbicide = (crop: GrainsCropTransformed, context: ExecutionContext<ConstantsForGrainsCalculator>) => {
    const zeroCO2 = rootOrigin(mass('CO2e', new Decimal(0)), { name: 'zero', valueType: 'constant' });
    const herbicide: Output<3, 'CO2e'> = output('herbicide', 3, zeroCO2);
    return {
        herbicide,
    }
}