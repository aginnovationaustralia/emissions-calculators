...
export function calculateScope1N2O(
  ...
  const {
    ...
    aboveGroundN,
    belowGroundN,
  } = constants.CROPRESIDUE[crop.type];
  ...
  const intermediaryBurnt = burnt0.includes(crop.type)
    ? 0
    : constants.CROPRESIDUE_PROPORTIONBURNT[crop.state].burnt;

  // (cropResiduesC12, cropResiduesK67:K82)
  // for sugar, use CROPRESIDUE_FRACTIONSUGARCANEBURNT instead
  const fractionBurnt =
    crop.type === 'Sugar Cane'
      ? constants.CROPRESIDUE_FRACTIONSUGARCANEBURNT[crop.state].burnt
      : intermediaryBurnt;
  ...
  const leachingN2OProductionEF = constants.LEACHING.N2O_EF;
  ...
  const pastureType = crop.type as keyof typeof constants.PASTURE_ATTRIBUTES;
